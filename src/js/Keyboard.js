import { KEYBOARD_MAP } from './keysData';
...
export default class Keyboard {
  ...
  setKeysText = () => {
    ...
    Object.keys(keyStorage).forEach((keyCode) => {
      ...
      } else if (this.isCapsLock && !key.keyData.noCaps) {
        key.setContent(this.lang, 'shifted');
      } else {
        key.setContent(this.lang);
      }
  ...
  initClickHandlers = () => {
    const keyStorage = this.domElements.keyBase;
    Object.keys(keyStorage).forEach((keyCode) => {
      const key = this.domElements.keyBase[`${keyCode}`].getNode();
